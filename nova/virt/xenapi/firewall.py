...
class Dom0IptablesFirewallDriver(IptablesFirewallDriver):
    ...
    def _plugin_execute(self, *cmd, **kwargs):
        ...
        args = {}
        LOG.debug("ORIGINAL ARGUMENTS:%s", kwargs)
        # NOTE (salvatore-orlando): this could be funny
        # if an iterable is passed as an argument
        args.update(map(lambda x: (x, str(kwargs[x])), kwargs))
        args['cmd_args'] = json.dumps(cmd)
        LOG.debug("PLUGIN ARGUMENTS:%s", args)
        ...
        LOG.debug("RETURNING:%s", json_ret)
